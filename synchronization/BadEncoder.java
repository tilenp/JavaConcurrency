...
public class BadEncoder {
    ...
    private AtomicInteger encodedNumber = new AtomicInteger(0);
    ...
    public void encode(int number) {
        if (number != lastNumber.get()) {
            expensiveOperation(number);
        }
        storage.store(lastNumber.get(), encodedNumber.get());
    }

    private void expensiveOperation(int number) {
        lastNumber.getAndSet(number);
        try {
            Thread.sleep(200);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        encodedNumber.getAndSet(number * 10);
    }
    ...
    public static void main(String[] args) {
        BadEncoder badEncoder = new BadEncoder();
        badEncoder.invoke();
    }
}